...
static inline size_t PageSize() { return static_cast<size_t>(getpagesize()); }
...
size_t page_size() { return PageSize(); }
...
FileMappingHandle CreateFileMappingHandle(const std::filesystem::path& path,
  ...
  std::string shm_name = MakeShmName(path);
  int fd = shm_open(shm_name.c_str(), oflag, 0777);
  if (fd < 0) {
    return kFileMappingHandleInvalid;
  }
