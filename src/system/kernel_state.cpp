...
#include <rex/system/kernel_constants.h>
...
void KernelState::SetExecutableModule(object_ref<UserModule> module) {
  ...
  auto export_entry = emulator_->processor()->export_resolver()->GetExportByOrdinal(
      "xboxkrnl.exe", system::kOrdinal_XexExecutableModuleHandle);
  if (export_entry) {
  ...
  export_entry = emulator_->processor()->export_resolver()->GetExportByOrdinal(
      "xboxkrnl.exe", system::kOrdinal_ExLoadedImageName);
  if (export_entry) {
