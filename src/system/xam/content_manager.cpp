...
ContentPackage::ContentPackage(KernelState* kernel_state, const std::string_view root_name,
    ...
    : kernel_state_(kernel_state), root_name_(root_name), package_path_(package_path) {
...
std::filesystem::path ContentManager::ResolvePackageRoot(XContentType content_type,
                                                         uint32_t title_id) {
...
std::filesystem::path ContentManager::ResolvePackagePath(const XCONTENT_AGGREGATE_DATA& data) {
  // Content path:
...
std::vector<XCONTENT_AGGREGATE_DATA> ContentManager::ListContent(uint32_t device_id,
...
X_RESULT ContentManager::GetContentThumbnail(const XCONTENT_AGGREGATE_DATA& data,
...
X_RESULT ContentManager::SetContentThumbnail(const XCONTENT_AGGREGATE_DATA& data,
                                             std::vector<uint8_t> buffer) {
  auto global_lock = global_critical_region_.Acquire();
  auto package_path = ResolvePackagePath(data);
